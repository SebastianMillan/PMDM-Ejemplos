...
import { VehicleResult, vehicle } from 'src/app/modules/vehicle.interface';
...
@Component({
  ...
  styleUrls: ['./card-vehicles.component.css']
})
export class CardVehiclesComponent {
  lista = ListVehicles;
  num = 0
  idVehicle = "";

  putImg(v: vehicle) {
    this.idVehicle = v.url.trim().split('/')[v.url.trim().split('/').length-2];
    return "https://starwars-visualguide.com/assets/img/vehicles/"+this.idVehicle+".jpg"
  }
